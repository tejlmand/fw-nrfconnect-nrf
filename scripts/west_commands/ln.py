...
class Ln(WestCommand):
    ...
    def do_add_parser(self, parser_adder):
        ...
        group.add_argument(
            '--status', action='store_true', help='''Show current status of links out of sync.'''
        )
        group.add_argument(
            '--sync',
            action='store_true',
    ...
    def _git(self, repo, command, args):
        cp = subprocess.run(
    ...
    def duplicates(self, args, repo, links):
        ...
        duplicates = []
        target_id = self._git(repo, 'hash-object', [target]).pop()
        tree_objects = self._git(repo, 'ls-tree', ['-r', 'HEAD'])
    ...
    def get_staged_links(self, repo, links):
        files = self._git(repo, 'diff', ['--cached', '--name-only'])
        return list(set(files) & set(links.keys()))

    def get_commited_links(self, repo, commit_range, links):
        files = self._git(repo, 'diff', [commit_range, '--name-only'])
        return list(set(files) & set(links.keys()))
    ...
    def status(self, args, repo, links):
        ...
        if args.target:
            commit_range = args.target
            files = self.get_commited_links(repo, commit_range, links)
        else:
        ...
        for file in files:
            ...
            result = set(self._git(repo, 'hash-object', registered_linked_files))
            if len(result) > 1:
                ...
                result = self._git(repo, 'hash-object', other_linked_files)
                for other_file, sha in zip(other_linked_files, result, strict=True):
                    state[other_file] = '+' if sha == ref_sha else '-'

        print(
            f"Linked files status, *=synced ({'commited' if args.target else 'staged'}), "
    ...
    def sync(self, args, repo, links):
        ...
        for file in files:
            ...
            result = set(self._git(repo, 'hash-object', staged_linked_files))
            if len(result) > 1:
                ...
                self._git(repo, 'add', other_linked_files)
