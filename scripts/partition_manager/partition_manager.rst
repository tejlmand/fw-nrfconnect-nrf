...
Partition Manager is a Python script that sets the start address and size of all partitions in a multi-image and/or multi-domain build context.

.. note::
   A domain is defined by a start address and size.
   A partition must be unique within a domain.
   Typically, a domain corresponds to a SoC.

When creating an application that require partitions for child images (for example, a bootloader) and/or subsystems (for example file systems) and/or domains (for example in a multi core solution) you can configure Partition Manager to control where in memory each partition should be placed.
...
The |NCS| build system creates a list of :file:`pm.yml` configuration files per domain as it traverses the images and subsystems included in a build.
These files are given to the Partition Manager script which deduces the start address and size of each partition.
...
.. note::
   ...
   Each domain defines its own partition namespace.
   Hence, :file:`pm.yml` files can only reference partitions contained within the domain.
...
:file:`pm.yml` files are added to the current domain configuration list.
By default, the current domain corresponds to the ``${BOARD}`` CMake variable.
...
The script is executed once per domain.
...
This means it contains the merged contents of all :file:`pm.yml` files, the sizes and addresses of all partitions in that domain, and other information generated by the Partition Manager.
...
Generated output
================
After the main Partition Manager script has finished for all partitions, another script runs.
This script takes the :file:`partitions.yml` files as input and creates the following output files:
...
Both kinds of files contain, among other information, the start address and size of all partitions for all domains.
...
The generated :file:`pm_config.h` file contains information for all domains.
When using only the partition names to look up a value, the macro will look up the value using the ``CONFIG_BOARD`` as a key.
It is possible to directly look up a value for a specific domain by using the full domain-prepended key.
...
.. code-block:: cmake
   ...
   pm_set_hex(app ${PROJECT_BINARY_DIR}/signed.hex signed_target)

   macro(pm_set_hex partition_name file file_target)
      set_property(
         GLOBAL PROPERTY
         ${BOARD}_${partition_name}_PM_HEX_FILE
         ${file}
      )

      set_property(
         GLOBAL PROPERTY
         ${BOARD}_${partition_name}_PM_TARGET
         ${file_target}
      )
   endmacro()
...
.. code-block:: none
   ...
   --slot-size $<TARGET_PROPERTY:partition_manager,${BOARD}PM_MCUBOOT_PARTITIONS_PRIMARY_SIZE> # TODO correct?
...
If multiple domains are included in the build, the files providing the static configuration must have the board name suffixed, e.g. :file:`pm_static_nrf5340_dk_nrf530_cpuapp.yml`.

The current partition configuration for a build can be found in :file:`${BUILD_DIR}/partitions_${BOARD}.yml`.
